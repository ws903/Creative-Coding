...
function draw()
{
  ...
  for (i = 0; i<sines.length; i++) // go through all the sines
  {
    ...
    if (!trace) ellipse(0, 0, radius*2, radius); // if we're simulating, draw the sine
    push(); // go up one level
    translate(0, radius); // move to sine edge
    if (!trace) ellipse(0, 0, 5, 2.5); // draw a little circle
    if (trace) ellipse(0, 0, erad, erad/2); // draw with erad if tracing
